          ...
          {characters.map(char => (
            <div key={char.id} className="bg-slate-800 rounded-xl overflow-hidden border border-slate-700 group hover:border-pink-500/50 transition-all">
              <div className="aspect-[3/4] bg-slate-900 relative">
                {char.imageUrl ? (
                  ...
                  <div className="w-full h-full flex flex-col items-center justify-center text-slate-600 p-6 text-center">
                    ...
                    <p className="text-sm">Concept Art Pending</p>
                  </div>
                )}
              ...
              <div className="p-4 space-y-3">
                ...
                <p className="text-xs text-slate-400 line-clamp-3 leading-relaxed">{char.description}</p>

                <button
                  onClick={() => onGenerateCharacter(char.id)}
                  disabled={char.isGenerating}
                  className="w-full py-2 bg-slate-700 hover:bg-pink-600 text-white text-sm rounded-lg flex items-center justify-center gap-2 transition-colors disabled:opacity-50 mt-2"
                >
                  {char.imageUrl ? <RefreshCw className="w-4 h-4" /> : <Wand2 className="w-4 h-4" />}
                  {char.imageUrl ? 'Regenerate' : 'Generate Concept'}
                </button>
              </div>
            </div>
          ))}
